...
void Joypick::pollJoystick()
{
    joy->getdata();
        ...
        std::vector<RobotCommands> KOMENDY;
        KOMENDY.clear();
        ...
        RobotCommands Robocik = {
            0, //nie wiem skąd brać IDrobota więc prowizorycznie daje 0
            (joy->axis[4]) * 0.000915,  // ta stala powoduje ze MAX wartosc w strukturze jest mniejsza niz 30
            (joy->axis[3]) * 0.000915,
            (joy->axis[0]) * 0.000915,
        };
        KOMENDY.push_back(Robocik);
